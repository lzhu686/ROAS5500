...
@dataclass
class CameraConfig:
    width: int = 320  # Reduced for faster upload
    height: int = 240  # Reduced for faster upload
    snapshot_path: str = "/tmp/garbage_snapshot.jpg"
    jpeg_quality: int = 70  # Lower quality = smaller file = faster upload


@dataclass
class ServerConfig:
    url: str
    timeout: int = 10  # Reduced timeout
    connect_timeout: float = 2.0  # Fast connection timeout
...
class PhotoClassifier:
    def __init__(self, cam_cfg: CameraConfig, server_cfg: ServerConfig) -> None:
        ...
        # Use session for connection reuse (faster subsequent requests)
        self.session = requests.Session()
        # Pre-warm connection
        try:
            self.session.head(server_cfg.url, timeout=2)
        except:
            pass
    ...
    def capture_to_file(self) -> str:
        ...
        # Save with specified JPEG quality (lower = smaller file)
        img.save(snapshot_path, quality=self.cam_cfg.jpeg_quality)
        ...
        with open(image_path, "rb") as stream:
            try:
                response = self.session.post(
                    ...
                    timeout=(self.server_cfg.connect_timeout, self.server_cfg.timeout),
                )
...
def build_default_config() -> AssistantConfig:
    ...
    cfg = AssistantConfig(
        ...
        camera=CameraConfig(),  # Uses optimized defaults (320x240, quality=70)
        server=ServerConfig(url="http://10.4.0.3:8000/classify", timeout=10, connect_timeout=2.0),
        category_phrase_ids={
            "可回收物": 1,
            "厨余垃圾": 2,
            "有害垃圾": 3,
